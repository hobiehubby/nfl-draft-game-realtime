document.addEventListener('DOMContentLoaded', () => {
    ...
    const shareLinkInput = document.getElementById('shareLinkInput'); // Needed for copy listener
    const copyLinkButton = document.getElementById('copyLinkButton');    // Needed here to check existence
    ...
    let copyLinkListenerAttached = false; // Flag for copy listener
    ...
    socket.on('gameCreated', ({ gameId, expectedPlayers }) => {
        ...
        // Ensure copy listener is attached when lobby is shown
    ...
    socket.on('joinSuccess', ({ username, gameState, draft }) => {
        ...
        clearStatus(loginStatus); if (usernameInput) usernameInput.value = ''; // Clear input if exists
    ...
    function updateUIBasedOnState() {
        ...
        if (currentPhase !== 'lobby') copyLinkListenerAttached = false; // Reset copy listener flag
        ...
        switch (currentPhase) {
            ...
            case 'lobby':
                ...
                attachCopyLinkListener(); // Attach copy listener here
                break;
            case 'login':
    ...
    // No changes needed in these core draft board functions from last version
    ...
    // No changes needed in these core draft state functions
    ...
    function attachLoginListeners() {
         if (loginListenersAttached || !loginButton || !usernameInput) return;
         console.log("DEBUG: Attaching Login listeners.");
         loginButton.addEventListener('click', () => {
             console.log("DEBUG: Join Game button CLICKED!"); const username = usernameInput.value.trim();
             ...
             else if (!currentGameId) { console.log("DEBUG: Join fail - No gameId"); displayStatus('Error: No game ID.', loginStatus); }
    ...
    function attachCopyLinkListener() { // Now defined as a separate function
         if (copyLinkListenerAttached || !copyLinkButton || !shareLinkInput) return;
         console.log("DEBUG: Attaching Copy Link listener.");
         copyLinkButton.addEventListener('click', () => {
             console.log("DEBUG: Copy Link button CLICKED!"); shareLinkInput.select(); shareLinkInput.setSelectionRange(0, 99999);
             try {
                 let copied = false;
                 if (navigator.clipboard) {
                    navigator.clipboard.writeText(shareLinkInput.value).then(() => { copied = true; alert('Link copied!'); }).catch(err => { console.warn('Clipboard API fail:', err); copied = document.execCommand('copy'); if (copied) { alert('Link copied (fallback).'); } else { throw new Error('execCommand fail'); } });
                 } else { copied = document.execCommand('copy'); if (copied) { alert('Link copied (fallback).'); } else { throw new Error('execCommand fail'); } }
             } catch (err) { console.error('Copy failed:', err); alert('Copy failed. Please copy manually.'); }
         });
         copyLinkListenerAttached = true; // Set flag
    }

    function attachDraftingListeners() { // Consolidated drafting listeners here
         // Check if listeners already exist might be needed if called multiple times
         ...
         // Add flags if needed to prevent duplicates
    }
